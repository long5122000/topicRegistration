...
const BaseConfirmationDetail = () => {
  ...
  const imageUrl = getValues("baseconfirmationImage");
  ...
  useEffect(() => {
    async function getData() {
      const colRef = collection(db, "BaseConfirmationList");
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const colRef = doc(db, "BaseConfirmations", confirmationId);
  ...
  const handleConfirmation = async (values: values): Promise<void> => {
    ...
    try {
      students.forEach(async (item) => {
        await deleteDoc(doc(db, "BaseConfirmationList", item.id));
      });
      const colRef = collection(db, "BaseConfirmationList");
      await addDoc(colRef, {
...
export default BaseConfirmationDetail;
