...
const InternshipBaseOpinionList = () => {
  ...
  const imageUrl = userInfo.baseOpinionImage;
  ...
  const handleDeledeConfirmation = async (item: any) => {
    ...
    //   return;
    // }
    const colRef = doc(db, "InternshipBaseOpinions", item.id);
    Swal.fire({
      title: "Are you sure?",
    ...
    <tr key={topic.id}>
      ...
      <td>
        <div className="flex items-center gap-x-3 text-gray-500">
          <ActionView
            onClick={() =>
              navigate(`/InternshipBaseOpinionLecturerView?id=${topic.id}`)
            }
          ></ActionView>
  ...
  return (
    <div className="container">
      <div className="flex justify-end my-5 ">
        <Button className="" kind="primary" href="/InternshipBaseOpinionAddNew">
          Tạo mới cuộc hẹn
        </Button>
      </div>
      <Table>
...
export default InternshipBaseOpinionList;
