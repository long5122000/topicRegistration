...
const InternshipReportDetail = () => {
  ...
  const fileUrl = getValues("internshipReportFile");
  ...
  useEffect(() => {
    async function getData() {
      const colRef = collection(db, "InternshipReportList");
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const colRef = doc(db, "InternshipReports", confirmationId);
  ...
  const handleConfirmation = async (values: values): Promise<void> => {
    ...
    try {
      students.forEach(async (item) => {
        await deleteDoc(doc(db, "InternshipReportList", item.id));
      });
      const colRef = collection(db, "InternshipReportList");
      await addDoc(colRef, {
      ...
      await updateDoc(colRef1, {
        internshipReportFile: file,
        internshipReportFileName: fileName,
      });
