...
const InternshipReportList = () => {
  ...
  const handleDeledeConfirmation = async (item: any) => {
    ...
    //   return;
    // }
    const colRef = doc(db, "InternshipReports", item.id);
    Swal.fire({
      title: "Are you sure?",
    ...
    <tr key={topic.id}>
      ...
      <td>
        <div className="flex items-center gap-x-3 text-gray-500">
          <ActionView
            onClick={() =>
              navigate(`/InternshipReportLecturerView?id=${topic.id}`)
            }
          ></ActionView>
  ...
  return (
    <div className="container">
      <div className="flex justify-end my-5 ">
        <Button className="" kind="primary" href="/InternshipReportAddNew">
          Tạo mới cuộc hẹn
        </Button>
      </div>
      <Table>
